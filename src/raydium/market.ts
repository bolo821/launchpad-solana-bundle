import { CacheLTA, InstructionType, LOOKUP_TABLE_CACHE, MAINNET_PROGRAM_ID, MARKET_STATE_LAYOUT_V2, MarketV2, TOKEN_PROGRAM_ID, Token, TxVersion, buildSimpleTransaction, generatePubKey, splitTxAndSigners, struct, u16, u32, u64, u8 } from "@raydium-io/raydium-sdk";
...
export const createOpenBookMarketOld = async (connection: Connection, payer: Keypair, mint: PublicKey, decimals: number, minOrderSize: number, tickSize: number) => {
...
export async function createOpenBookMarket(connection: Connection, payer: Keypair, baseMintAddress: string, quoteMintAddress: string, lotSize: number, tickSize: number) {
    console.log("Creating OpenBook Market...", baseMintAddress, lotSize, tickSize, MAINNET_PROGRAM_ID.OPENBOOK_MARKET.toBase58());
    ...
    const marketAccounts = await Market.findAccountsByMints(connection, baseMint, quoteMint, MAINNET_PROGRAM_ID.OPENBOOK_MARKET);
