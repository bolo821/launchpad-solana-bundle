...
const main = async () => {
	const TOKEN_NAME = 'beefboy';
	const TOKEN_SYMBOL = 'BEEFBOY';
	const DECIMALS = 9;
	const TOTAL_SUPPLY = 1000000;
	const TOKEN_IMG_NAME = 'test.png';
	const TOKEN_DESCRIPTION = 'beef boy';
	...
	const bals = await Promise.all(allkeypairs.map(async k => {
		return await connection.getBalance(k.publicKey)
	}))

	if (bals.find(b => b === 0) !== undefined) return
	...
	if (!tokenAddress) {
		...
		fs.writeFileSync('1.json', JSON.stringify({token: tRet?.toString()}, null, '\t'))
		...
		if (tokenAddress) {
			try {
				await createOpenBookMarket(connection, payer, tokenAddress, Token.WSOL.mint.toString(), 1, 0.000001);
			} catch (err) {
	...
	const poolCreationPromise = new Promise((resolve, reject) => {
		/// pool creation
		createPoolTxBuild(connection, payer, tokenMint as PublicKey, BigInt(totalTokenAmountToPool * (10 ** 9)), BigInt(Math.floor(totalSolAmountToPool * (10 ** 9))))
	...
	const buyPromises = buyList.map(async buyer => {
		const inputTokenAmount = new TokenAmount(Token.WSOL, BigInt(buyer.solAmount * (10 ** 9)));
		const outputToken = new Token(TOKEN_PROGRAM_ID, tokenMint as PublicKey, DECIMALS);
		const buy1Array: (VersionedTransaction | Transaction)[] = await buildSwapTransactions(connection, buyer.signer, inputTokenAmount, outputToken, true)
		if (buy1Array === null || buy1Array.length !== 1) {
			return null
		} else {
			const txn = buy1Array[0] as VersionedTransaction
			txn.sign([buyer.signer])
			console.log(`buy ${buyer.signer.publicKey.toString()}`, bs58.encode(txn.signatures[0]))
			return txn
		}
	})
